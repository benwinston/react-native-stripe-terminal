        ...
        if (selectedReader != null) {
            BluetoothConnectionConfiguration config = new BluetoothConnectionConfiguration(locationId, true, new BluetoothReaderReconnectionListener() {
                ...
                public void onReaderReconnectStarted(@Nonnull Cancelable cancelReconnect) {
                    ...
                    sendEventWithName(EVENT_READER_RECONNECTION_STARTED, Arguments.createMap());
                }
                ...
                public void onReaderReconnectSucceeded() {
                    ...
                    sendEventWithName(EVENT_READER_RECONNECTION_SUCCEEDED, Arguments.createMap());
                }
        ...
        if (pendingCreatePaymentIntent != null && !pendingCreatePaymentIntent.isCompleted()) {
            pendingCreatePaymentIntent.cancel(new Callback() {
                ...
                public void onFailure(@Nonnull TerminalException e) {
                    WritableMap errorMap = Arguments.createMap();
                    errorMap.putString(ERROR, e.getErrorMessage());
                    sendEventWithName(EVENT_ABORT_CREATE_PAYMENT_FAILED, errorMap);
                }
            });
        } else {
            sendEventWithName(EVENT_ABORT_CREATE_PAYMENT_NOTHING_TO_DO, Arguments.createMap());
        }
    }
